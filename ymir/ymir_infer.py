...
def mmdet_result_to_ymir(results: DetDataSample, class_names: List[str]) -> List[rw.Annotation]:
    ...
    for idx, result in enumerate(zip(bboxes, scores, labels)):
        ...
        xc, yc, w, h, angle = [x.item() for x in bbox]
        x1 = xc - w / 2
        y1 = yc - h / 2
        x2 = xc + w / 2
        y2 = yc + h / 2
...
class YmirModel:
    ...
    def __init__(self, cfg: edict):
        ...
        mmcfg = Config.fromfile(config_file)
        # inference_detector will use mmcfg.test_dataloader.dataset.pipeline
        mmcfg.test_dataloader.dataset.pipeline = mmcfg.test_pipeline
        self.model = init_detector(mmcfg, checkpoint_file, device=f'cuda:{gpu_id}', cfg_options=cfg_options)
